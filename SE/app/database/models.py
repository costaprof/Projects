...
class PizzaType(Base):
    ...
    sauce_id: Mapped[uuid.UUID] = mapped_column(ForeignKey('sauce.id'), nullable=True)
    ...
    def __repr__(self):
        return "PizzaType(id='%s', name='%s', price='%s', description='%s', sauce='%s', type='%s')" \
            % (self.id, self.name, self.price, self.description, self.sauce, self.type)
...
class User(Base):
    ...
    customer_orders: Mapped[List['Order']] = \
        (relationship(cascade='all, delete-orphan', back_populates='user'))  # NOSONAR
