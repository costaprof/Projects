...
from app.api.v1.endpoints.order.schemas \
    import (OrderSchema,
            ...
            PizzaWithoutPizzaTypeSchema,
            OrderBeverageQuantityCreateSchema,
            JoinedOrderBeverageQuantitySchema,
            OrderPriceSchema, OrderBeverageQuantityBaseSchema,
            OrderCreateSchema)
...
def get_all_orders(
        order_status: Optional[str] = Query(None, description='The status of the orders to filter.'
                                                              ' Possible values are: '
                                                              'transmitted, preparing, in_delivery, completed.'),
        db: Session = Depends(get_db),
):
    if order_status:
        order_status = order_status.upper()
        if order_status not in [order_status_temp.value for order_status_temp in OrderStatus]:
            logging.warning(f'Unprocessable parameter for order_status with value: {order_status}')
            raise HTTPException(status_code=422, detail='Unprocessable order_status')

    orders = order_crud.get_all_orders(db, order_status)
    if not order_status:
        order_status = 'NO PARAMETER ENTERED'
    logging.info(f'Returned all orders with parameter of value: {order_status}')
    return orders
...
def create_order(order: OrderCreateSchema, db: Session = Depends(get_db),
    ...
    if not copy_order:
        ...
        logging.info('Copy Order not found')
        raise HTTPException(status_code=404)
    ...
    for pizza in copy_order.pizzas:
        ...
        if not stock_ingredients_crud.ingredients_are_available(pizza_type):
            # Not enough Stock
            order_crud.delete_order_by_id(new_order.id, db)
            logging.info(f'Not enough Stock for Pizza {pizza_type.name}')
    ...
    for beverage_quantity in copy_order.beverages:
        schema = OrderBeverageQuantityCreateSchema(
            **{'quantity': beverage_quantity.quantity,
        ...
        if not stock_beverage_crud.change_stock_of_beverage(beverage_quantity.beverage_id,
            ...
            # Not enough Stock
            order_crud.delete_order_by_id(new_order.id, db)
            logging.info(f'Not enough Stock for Beverage {beverage_quantity.beverage.name}')
            raise HTTPException(status_code=409, detail='Conflict')
...
def update_order(
        order_id: uuid.UUID,
        order_status: Optional[str] = Query(None, description='The new status of the order.'
                                                              ' Possible values are: '
                                                              'transmitted, preparing, in_delivery, completed.'),
        db: Session = Depends(get_db),
):
    ...
    if not order_status:
        logging.warning('No value for parameter order_status was entered')
    ...
    # Update the order
    new_order_status = order_crud.update_order_status(order, OrderStatus[order_status].value, db)
    if new_order_status:
        logging.info(f'Order status updated with value: {order_status}')
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    else:
        logging.warning(f'Failed to update order status with value: {order_status}')
        return Response(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)
...
def delete_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.info(f'Order with id {order_id} not found to delete')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
...
def add_pizza_to_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.info(f'Order with id {order_id} not found for order')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
...
def get_pizzas_from_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.info(f'Order with id {order_id} not found for pizzas')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
...
def delete_pizza_from_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.info(f'Order with id {order_id} not found for deleting pizza')
        return Response(status_code=status.HTTP_404_NOT_FOUND)

    pizza_entity = order_crud.get_pizza_by_id(pizza.id, db)
    if not pizza_entity:
        logging.info(f'Pizza with id {pizza.id} not found to delete')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
    ...
    logging.info(f'Pizza with id {pizza.id} deleted successfully')
...
def get_order_beverages(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.info(f'Order with id {order_id} not found for beverages')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
...
def create_order_beverage(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.info(f'Order with id {order_id} not found for creating beverage')
        return Response(status_code=status.HTTP_404_NOT_FOUND)

    # Check if new Quantity is valid
    if beverage_quantity.quantity <= 0:
        logging.info('Quantity of Beverage must be greater than 0 when ordering')
        raise HTTPException(status_code=422)

    beverage = beverage_crud.get_beverage_by_id(beverage_quantity.beverage_id, db)
    if not beverage:
        logging.info(f'Beverage with id {beverage_quantity.beverage_id} not found')
        raise HTTPException(status_code=404)

    beverage_quantity_found = order_crud.get_beverage_quantity_by_id(
        order_id,
        beverage_quantity.beverage_id,
        db)
    if beverage_quantity_found:
        url = request.url_for('get_order_beverages',
                              order_id=beverage_quantity_found.order_id)
        logging.info(f'Beverage with id {beverage_quantity.beverage_id} '
                     f'already exists in order {order_id}')
        return RedirectResponse(url=url, status_code=status.HTTP_303_SEE_OTHER)
    # Change Stock of Beverage if enough is available
    if not stock_beverage_crud.beverage_is_available(beverage_quantity.beverage_id,
                                                     beverage_quantity.quantity,
                                                     db):
        logging.info(f'Not enough Stock for Beverage {beverage.name}')
        raise HTTPException(status_code=409, detail='Conflict')
    stock_beverage_crud.change_stock_of_beverage(beverage_quantity.beverage_id,
                                                 -beverage_quantity.quantity,
                                                 db)
    new_beverage_quantity = order_crud.create_beverage_quantity(order,
                                                                beverage_quantity,
                                                                db)
...
def update_beverage_of_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.warning(f'Order with id {order_id} not found for updating beverage')
        return Response(status_code=status.HTTP_404_NOT_FOUND)

    # Check if new Quantity is valid
    if beverage_quantity.quantity <= 0:
        logging.info('Quantity of Beverage must be greater than 0 when updating order')
        return Response(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)

    beverage_id = beverage_quantity.beverage_id
    order_beverage_quantity = order_crud.get_beverage_quantity_by_id(order_id, beverage_id, db)
    if not order_beverage_quantity:
        logging.info(f'Beverage with id {beverage_id} not found in order {order_id}')
        raise HTTPException(status_code=404)
    ...
    if not stock_beverage_crud.change_stock_of_beverage(beverage_id, old_quantity - new_quantity, db):
        logging.info(f'Not enough Stock for Beverage {order_beverage_quantity.beverage.name}')
        raise HTTPException(status_code=409, detail='Conflict')
    # Update
    ...
    if new_order_beverage_quantity:
        logging.info(f'Beverage with id {beverage_id} updated in order {order_id}')
        return new_order_beverage_quantity
    else:
        logging.warning(f'Beverage with id {beverage_id} not found in order {order_id}')
        raise HTTPException(status_code=404)
...
def delete_beverage_from_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.warning(f'Order with id {order_id} not found for deleting beverage')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
    ...
    logging.info(f'Beverage with id {beverage_id} deleted from order {order_id}')
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
def get_price_of_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.warning(f'Order with id {order_id} not found for price')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
...
def get_user_of_order(
    ...
    order = order_crud.get_order_by_id(order_id, db)
    if not order:
        logging.warning(f'Order with id {order_id} not found for user')
        return Response(status_code=status.HTTP_404_NOT_FOUND)
