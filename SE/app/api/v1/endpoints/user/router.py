...
def get_all_users(
    ...
    logging.info('router: retrieved all users')
    return users
...
def create_user(user: UserCreateSchema, db: Session = Depends(get_db)):
    ...
    logging.info(f'router: created new user: {new_user}')
    return new_user
...
def update_user(
    ...
    if user_found:
        ...
        logging.info(f'router: updated user: {user_found}')
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    else:
        logging.error(f'router: could not find user: {user_id}')
...
def get_user(user_id: uuid.UUID,
    ...
    logging.info(f'router: retrieved user: {user_found}')
    return user_found
...
def delete_user(
    ...
    logging.info(f'router: deleted user: {user_found}')
    return Response(status_code=status.HTTP_204_NO_CONTENT)
